...
`open_items` is the count of **open issues with the `pull_request` key absent** — open PRs are excluded so they don't inflate the backlog. ETA is `open_items / per_day`, expressed as days and an absolute date.
